...
- [ ] add custom rules for each var selectable from a select element if nedeed (example: chars always continue drawing, but in some drawings x and y do nothing).
...
- [ ] implement auto-zoom in the auto-center function so the drawing fits inside the canvas
